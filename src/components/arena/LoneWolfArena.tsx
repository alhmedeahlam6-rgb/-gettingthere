        ...
        if (!human.alive) {
          ...
          if (move.lengthSq() > 0) {
            ...
            if (colliders.length > 0) {
              const probeFar = PLAYER_RADIUS + speed + 0.05;
              const probe = (dir: THREE.Vector3) => {
                // knee first: most obstacles are hit there, and the moment one
                // blocks us the taller samples can't change the outcome.
              ...
              const hit = probe(dir);
              if (hit) {
                // Colliders are DoubleSide, so a flipped face can hand back a
                // normal pointing away from us. Orient it against the ray and
                // drop near-horizontal surfaces (ground/slopes/ceilings): their
                // XZ projection is tiny and normalizing it yields an arbitrary
                // sideways vector, which reads as the camera being punched.
                const raw = hit.face
                  ? hit.face.normal.clone().transformDirection(hit.object.matrixWorld).normalize()
                  : dir.clone().negate();
                if (raw.dot(dir) > 0) raw.negate();
                const isWall = Math.abs(raw.y) < 0.7;
                const n = isWall ? raw.clone().setY(0).normalize() : null;
                if (n && n.lengthSq() > 0.01) {
